...
function LinearProgramming() {
  ...
  const solveGraphically = () => {
    // Note: Graphical method only works for 2 variables
    alert('Graphical method is only available for problems with 2 variables. Please use the Simplex Method for 3 variables.');
    return;
  };
  ...
  const solveSimplex = () => {
    ...
    const obj = {
      x1: parseFloat(simplexFormData.objective.x1),
      x2: parseFloat(simplexFormData.objective.x2),
    ...
    const numLeConstraints = constraints.filter(c => c.sign === '<=').length;
    const artificialColumns = tableau[0].length - numLeConstraints - 2; // exclude original vars and RHS
    for (let j = tableau[0].length - artificialColumns - 1; j < tableau[0].length - 1; j++) {
      for (let i = 0; i < tableau.length - 1; i++) {
        if (Math.abs(tableau[i][j] - 1) < 0.000001 && tableau[i][tableau[0].length - 1] > 0.000001) {
          setSimplexSolution({ status: 'infeasible' });
          return;
        }
      }
    }
      ...
      <div className="theory-section">
        ...
        <div className="form-container">
          ...
          <div className="input-group">
            ...
            <select
              name="numVars"
              value={formData.objective.numVars}
                ...
                {formData.objective.numVars === 3 && (
                  <>
                    ...
                    <input
                      type="number"
                      value={constraint.x3}
                      onChange={(e) => handleConstraintChange(index, 'x3', e.target.value)}
                      placeholder="x₃ coefficient"
                    />
                    <span>x₃</span>
                  </>
          ...
          <div className="subtopic">
            ...
            <div className="form-container">
              ...
              <div className="input-group">
                ...
                <select
                  name="numVars"
                  value={simplexFormData.objective.numVars}
                  onChange={(e) => handleSimplexObjectiveChange({
                    target: {
                      name: 'numVars',
                      value: parseInt(e.target.value)
                    }
              ...
              <div className="input-group">
                ...
                <div className="objective-inputs">
                  ...
                  />
                  <span>x₂</span>
                  {simplexFormData.objective.numVars === 3 && (
                    <>
                      <span>+</span>
                      <input
                        type="number"
                        name="x3"
                        value={simplexFormData.objective.x3}
                        onChange={handleSimplexObjectiveChange}
                        placeholder="x₃ coefficient"
                      />
                      <span>x₃</span>
                    ...
                    {simplexFormData.objective.numVars === 3 && (
                      <>
                        ...
                        <input
                          type="number"
                          value={constraint.x3}
                          onChange={(e) => handleSimplexConstraintChange(index, 'x3', e.target.value)}
                          placeholder="x₃ coefficient"
                        />
                        <span>x₃</span>
                      </>
          ...
          <div className="subtopic">
            ...
            <div className="form-container">
              ...
              <div className="input-group">
                ...
                <select
                  name="numVars"
                  value={dualityFormData.objective.numVars}
                  onChange={(e) => handleDualityObjectiveChange({
                    target: {
                      name: 'numVars',
                      value: parseInt(e.target.value)
                    }
              ...
              {/* Objective Function */}
              <div className="input-group">
                <h4>Primal Objective Function</h4>
                <div className="objective-inputs">
                  <input
                    type="number"
                    name="x1"
                  ...
                  />
                  <span>x₂</span>
                  {dualityFormData.objective.numVars === 3 && (
                    <>
                      <span>+</span>
                      <input
                        type="number"
                        name="x3"
                        value={dualityFormData.objective.x3}
                        onChange={handleDualityObjectiveChange}
                        placeholder="x₃ coefficient"
                      />
                      <span>x₃</span>
                    ...
                    {dualityFormData.objective.numVars === 3 && (
                      <>
                        ...
                        <input
                          type="number"
                          value={constraint.x3}
                          onChange={(e) => handleDualityConstraintChange(index, 'x3', e.target.value)}
                          placeholder="x₃ coefficient"
                        />
                        <span>x₃</span>
                      </>
